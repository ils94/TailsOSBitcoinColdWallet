# TailsOS를 최신 상태로 유지하는 것은 비트코인을 도난당할 수 있는 악성 공격을 피하기 위해 매우 중요합니다! 만약 오래된 버전의 TailsOS를 사용한다면, 오프라인 지갑에 접근할 때 절대 인터넷에 연결하지 않도록 기억하세요! 온라인 지갑을 만들기 위해 두 번째 USB 드라이브를 사용하거나 안드로이드나 iOS에서 온라인 지갑을 만들기 위해 다른 수단을 사용하고, QR 코드로 거래를 서명하는 공기 갭 방법을 사용하세요. 이 경고를 무시하거나 다른 보안 규칙을 어기면 비트코인이 도난당할 경우 책임지지 않습니다.
