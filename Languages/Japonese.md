# TailsOSのバージョンを常に最新に保つことは非常に重要です。これにより、ビットコインを盗む可能性のある悪意のある攻撃を回避できます！古いバージョンのTailsOSを使用する場合は、オフラインウォレットにアクセスする際に決してインターネットに接続しないようにしてください！オンラインウォレットを作成するために第二のUSBドライブを使用するか、AndroidやiOSでオンラインウォレットを持つための他の手段を使用し、QRコードでトランザクションに署名するためのエアギャップメソッドを使用してください。この警告を無視することを含む、セキュリティルールを破った場合のビットコインの盗難に対しては一切責任を負いません。
